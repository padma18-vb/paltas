...
###############################
def matrix_plot_from_npy(file_list,names,dim,save_name,stretch='log',asinh_a=0.1, max_cut = None, annotate=False):
    """
    Args: 
    ...
    for i in range(0,dim[0]):
        ...
        for j in range(0,dim[1]):
            if file_list[file_counter] is None:
                ...
                # normalize data using log and min cutoff 
            norm = simple_norm(cropped_data,stretch=stretch,min_cut=1e-6, asinh_a=asinh_a, max_cut=max_cut)
